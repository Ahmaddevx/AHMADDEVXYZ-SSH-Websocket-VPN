...
# Constants
ahmaddevxyz_websocket_SERVICE="ahmaddevxyz-websocket"
PYTHON_SCRIPT_PATH="/opt/ahmaddevxyz_websocket/ahmaddevxyz_websocket.py"
...
check_server_status() {
   echo "Server Status:"
   systemctl is-active $ahmaddevxyz_websocket_SERVICE
}
...
restart_websocket_service() {
   echo "Restarting $ahmaddevxyz_websocket_SERVICE service..."
   systemctl restart $ahmaddevxyz_websocket_SERVICE
   systemctl status $ahmaddevxyz_websocket_SERVICE --no-pager  # Optionally, show status after restarting
}

# Function to uninstall proxy script
uninstall_proxy_script() {
   echo "Stopping $ahmaddevxyz_websocket_SERVICE service..."
   systemctl stop $ahmaddevxyz_websocket_SERVICE

   echo "Disabling $ahmaddevxyz_websocket_SERVICE service..."
   systemctl disable $ahmaddevxyz_websocket_SERVICE

   echo "Removing Python proxy files..."
   rm -rf "/opt/ahmaddevxyz_websocket"
   rm /usr/local/bin/websocket

   echo "Removing systemd service file..."
   rm -f "/etc/systemd/system/$ahmaddevxyz_websocket_SERVICE.service"
...
server_information() {
   ...
   if systemctl is-active --quiet $ahmaddevxyz_websocket_SERVICE; then
